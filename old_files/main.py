...
stepsize = 0.2
p1_values = np.arange(0.1, 1, stepsize)  # Values of p1 from 0 to 1
b1 = 1
b2 = 8
...
for p1 in p1_values:
    p2 = 1 - p1  # Since we only have b1 and b2
    pp_budgets = [b1] * int(p1 * num_samples) + [b2] * int(p2 * num_samples)
    

    
    print(f"\nOn sampling for p1 = {p1}")
...
cmap = cm.get_cmap('viridis', len(p1_values))

for i, p1 in enumerate(p1_values):
    color = cmap(i / len(p1_values))
    axes[0].plot(deltas, epsilon_sampling_b1[i], label=f'portion of {p1:.2f}', color=color)
    axes[1].plot(deltas, epsilon_sampling_b2[i], label=f'portion of {1-p1:.2f}', color=color)

axes[0].set_title(f'Sampling - epsilon {b1}')
axes[1].set_title(f'Sampling - epsilon {b2}')
...
plt.tight_layout()
plt.savefig('./epsilon_variation_plots_new.png')
