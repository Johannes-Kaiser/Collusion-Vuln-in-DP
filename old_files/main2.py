...
stepsize = 0.1
b1 = 1
b2_values = [2, 5, 10, 20, 50, 100, 500, 1000]  # Different values for b2
deltas = np.logspace(-12, -2, 400)
p1 = 0.5  # Fixed portion of p1
p2 = 1 - p1
...
for b2 in b2_values:
    pp_budgets = [b1] * int(p1 * num_samples) + [b2] * int(p2 * num_samples)
    
    print(f"\nOn clipping for b2 = {b2}")
    args.individualize = "clipping"
    clipping_data = make_private(dummy_model, train_loader, pp_budgets, args)
    
    print(f"\nOn sampling for b2 = {b2}")
...
cmap = cm.get_cmap('viridis', len(b2_values))

for i, b2 in enumerate(b2_values):
    color = cmap(i / len(b2_values))
    axes[0, 0].loglog(deltas, epsilon_clipping_b1[i], label=f'b2 = {b2}', color=color)
    axes[0, 1].loglog(deltas, epsilon_sampling_b1[i], label=f'b2 = {b2}', color=color)
    axes[1, 0].loglog(deltas, epsilon_clipping_b2[i], label=f'b2 = {b2}', color=color)
    axes[1, 1].loglog(deltas, epsilon_sampling_b2[i], label=f'b2 = {b2}', color=color)

axes[0, 0].set_title(f'Clipping - epsilon {b1}')
axes[0, 1].set_title(f'Sampling - epsilon {b1}')
axes[1, 0].set_title(f'Clipping - epsilon varying b2')
axes[1, 1].set_title(f'Sampling - epsilon varying b2')
...
plt.tight_layout()
plt.savefig('./epsilon_group_variation.png')
plt.show()
