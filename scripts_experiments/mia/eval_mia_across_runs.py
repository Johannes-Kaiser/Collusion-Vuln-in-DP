...
for all_paths_portion, portions in zip(all_paths, portions_list):
    ...
    for p in all_paths_portion:
        ...
        # Load numpy arrays
        try:
            info["num_steps_list"] = np.load(os.path.join(p, "num_steps_list.npy")).tolist()
        except FileNotFoundError:
            continue
        info["pg_noise_multiplier_list"] = np.load(os.path.join(p, "pg_noise_multiplier_list.npy")).tolist()
        info["pg_sample_rates_list"] = np.load(os.path.join(p, "pg_sample_rates_list.npy")).tolist()
...
for portion, info in extracted_info.items():
    ...
    for key in ["num_steps_list", "pg_noise_multiplier_list", "pg_sample_rates_list"]:
        ...
        if not values:
            continue
        first = values[0][0] if isinstance(values[0], list) and values[0] else values[0]
        if all(isinstance(v, list) and v and all(x == first for x in v) for v in values):
            extracted_info[portion][key] = first
        else:
