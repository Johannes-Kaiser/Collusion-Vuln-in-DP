...
from utils.signals import load_input_logits, load_input_labels, convert_signals, load_inputs, load_input_gradient
...
from utils.utils_general import parse_extra
from utils.utils_plot import plot_roc
...
def collect_signal_lira(models_path,
    ...
    Args:
        ...
        : (List[int]) : which indices to query
        log_dir (str): log_dir 
    ...
    def load_or_compute_logits():
        ...
        signal_path = f"{log_dir}/{signal_folder}/{primary_signal}/{model_index}.npy" # TODO
        ...
        targets = load_input_labels(log_dir)

        if np.isnan(model_logits[:,:]).sum()>0:
            print(f"load the model and compute logits for model {model_index}")
            ...
            print(f"loading logits for model {model_index}")
    ...
    if signal_name == "softmax_relative" :
        ...
        # If not, query it
        if torch.isnan(model_signal).sum()>0:
            ...
            for k in range(nb_augmentations):
                model_signal[:, k] = convert_signals(model_logits[:, k, :], targets, "softmax", temp=temperature)
        ...
        if torch.isnan(model_signal).sum()>0:
            ...
            for k in range(nb_augmentations):
                model_signal[:, k] = convert_signals(model_logits[:, k, :], targets, "soft-margin", temp=temperature, extra=extra)
        ...
        if torch.isnan(model_signal).sum()>0:
            ...
            for k in range(nb_augmentations):
                model_signal[:, k] = convert_signals(model_logits[:, k, :], targets, "taylor", temp=temperature, extra=extra)
        ...
        signal_path = f"{log_dir}/{signal_folder}/{primary_signal}/{model_index}.npy"
        ...
        if torch.isnan(model_signal).sum()>0:
            ...
            print(f"load the model and compute signals for model {model_index}")

            for k in range(nb_augmentations):
                print(model_logits.shape)
                model_signal[:, k] = convert_signals(model_logits[:, k, :], targets, "taylor-soft-margin", temp=temperature, extra=extra)

            np.save(signal_path, model_signal)
            return model_signal
        else:
            print(f"loading signals for model {model_index}")
            return model_signal
    ...
    ## Note: To quickly implement another attack you can use another elif signal_name == "signal_name"
        ...
        signal_path = f"{log_dir}/signal/{primary_signal}/{model_index}.npy"
        ...
        if torch.isnan(model_signal).sum()>0:
            model_logits, targets = load_or_compute_logits()
            for k in range(nb_augmentations):
                model_signal[:, k] = convert_signals(model_logits[:, k, :], targets, "log-logit-scaling", temp=0)
            np.save(signal_path, model_signal)
            return model_signal
        else:
            print(f"loading signals for model {model_index}")
        ...
        print(f"loading gradients for model {model_index}")
...
def aggregate_signals(signal_name, 
    ...
    Args:
        ...
        : (array): indices of the columns in 1, len(queried_signals) for which to compute membership (e.g. the first 1000)
    ...
    """
    target_signal = reference_signals
    ...
    for data_idx in range(reference_signals.shape[1]):
        in_data = reference_signals[reference_keep_matrix[:, data_idx], data_idx][:out_or_in_size]
        in_signals.append(
            in_data
        ) # selects the models that have data_idx as train
        if in_data.shape[0] < in_lowest:
            in_lowest = in_data.shape[0]
        out_data = reference_signals[~reference_keep_matrix[:, data_idx], data_idx][:out_or_in_size]
        out_signals.append(
            out_data
        ) # selects the models that don't have data_idx as train
        if out_data.shape[0] < out_lowest:
            out_lowest = out_data.shape[0]
    for data_idx in range(reference_signals.shape[1]):
        in_signals[data_idx] = in_signals[data_idx][:in_lowest, :]
        out_signals[data_idx] = out_signals[data_idx][:out_lowest, :]
    ...
    if signal_name == "softmax_relative" or signal_name == "taylor_softmax_relative" or signal_name == "sm_taylor_softmax_relative" or signal_name == "sm_softmax_relative": # relative attack
        ...
        if int(configs["audit"]["top_k"]) != -1 : # selects for each x a random set of population equal to int(configs["audit"]["top_k"])
            ...
            target_distances = kernel[:,population_indices] # shape : x population_indices
        ...
        if configs["audit"]["augmentation"] == "augmented": # using augmentation
                ...
                for k in tqdm(range(0, nb_augmentations), desc=f"Relative attack for each query..."): # computing attack scores for each augmentation
                    ...
                    if str2bool(configs["audit"]["offline"]):
                        if "ymedians" in configs["audit"].keys():
                            ...
                            prob_ratio_x = (target_signal[:, k].ravel() / (np.interp(mean_x, bin_centers, ymedian)))
                            ...
                            prob_ratio_x = (target_signal[:, k].ravel() / ((1+offline_a)/2 * mean_x + (1-offline_a) /2))
                            prob_ratio_z_rev = 1 / (target_signal[population_indices, k].ravel() / ((1+offline_a)/2 * mean_z + (1-offline_a) /2))
                    else:
                        prob_ratio_x = (target_signal[:, k].ravel() / (mean_x))
            ...
            if len(ref_signals.shape) == 3:
                mean_x = trim_mean(ref_signals[:,:, 0], proportiontocut=proptocut, axis=0)
                ...
                if str2bool(configs["audit"]["offline"]):
                    if "ymedians" in configs["audit"].keys():
                        ...
                        prob_ratio_x = (target_signal[:, 0].ravel() / (np.interp(mean_x, bin_centers, ymedian)))
                        prob_ratio_z_rev = 1 / (target_signal[population_indices, 0].ravel() / (np.interp(mean_z, bin_centers, ymedian)))
                    else:
                        prob_ratio_x = (target_signal[:, 0].ravel() / ((1+offline_a)/2 * mean_x + (1-offline_a) /2))
                ...
                if str2bool(configs["audit"]["offline"]):
                    if "ymedians" in configs["audit"].keys():
                        ...
                        prob_ratio_x = (target_signal.ravel() / (np.interp(mean_x, bin_centers, ymedian)))
                        prob_ratio_z_rev = 1 / (target_signal[population_indices].ravel() / (np.interp(mean_z, bin_centers, ymedian)))
                    else:
                        prob_ratio_x = (target_signal.ravel() / ((1+offline_a)/2 * mean_x + (1-offline_a) /2))
                    ...
                    prob_ratio_x = (target_signal.ravel() / (mean_x))
                    prob_ratio_z_rev = 1 / (target_signal[population_indices].ravel() / (mean_z)) # the inverse to compute quickly
            
            np.savez(f"{log_dir}/{configs['audit']['report_log']}/p_x", # to vizualize p_OUT vs. p_IN
                mean_x=mean_x, # p(x)
                mean_z=mean_z,
                true_mean_x=ref_signals[:,:].mean(0),
                true_mean_z=ref_signals[:,population_indices].mean(0),
                mean_x_in=in_signals[:,:].mean(1), # shape dataset_size x out_or_in_size x nb_augmentations
                mean_x_out=out_signals[:,:].mean(1),
        ...
        if True:
            ...
            if os.path.exists(f"{log_dir}/{configs['audit']['report_log']}/fusion.npz"):
                data = np.load(f"{log_dir}/{configs['audit']['report_log']}/fusion.npz")
                ...
                np.savez(
                    f"{log_dir}/{configs['audit']['report_log']}/fusion",
        ...
        in_signals = in_signals.detach().cpu().numpy()
        out_signals = out_signals.detach().cpu().numpy() # target indices x nb models x nb augs
        ...
        if configs["audit"]["augmentation"] == "augmented":
            ...
            if len(target_signal.shape) == 2:
                sc = target_signal[:, 0].reshape(-1, 1) # 50k x 1, no augmentation
                in_signals = in_signals[:,:,0]
                out_signals = out_signals[:,:,0]
            else:
                sc = target_signal # 50k x 1
...
if __name__ == "__main__":
    ...
    for seed in range(configs["train"]["seeds"]):
        configs["data"]["savedir_target"] = configs["data"]["savedir"] + f"/{str(seed)}/target"
        configs["data"]["target_dir"] = configs["data"]["savedir"] + f"/{str(seed)}/target"
        configs["data"]["reference_dir"] = configs["data"]["savedir"] + f"/{str(seed)}/target"
        configs["data"]["savedir_result"] = configs["data"]["savedir"] + f"/{str(seed)}/result"
        # Set the random seed, log_dir and inference_game
        torch.manual_seed(seed)
        np.random.seed(seed)

        log_dir = configs["data"]["savedir_result"]
        ...
        num_data = configs["data"]["dataset_size"]
        ...
        model_idx = os.listdir(target_path)
        ...
        targets = load_input_labels(log_dir) # target_path
        ...
        if target_path != reference_path:
            ...
            targets = load_input_labels(log_dir) # reference_path

        results_path = f"{log_dir}/{configs['audit']['report_log']}/attack_stats.npz"


        reference_signals = signals
        reference_keep_matrix = keep_matrix
        ...
        if configs["audit"]["signal"] in ["softmax_relative", "taylor_softmax_relative", "sm_taylor_softmax_relative", "sm_softmax_relative"]: # relative attacks (hardcoded)
            ...
            if int(configs["audit"]["top_k"]) != -1 :
                pdistances = torch.rand(num_data, num_data)
            else:
                pdistances = None
            ...
            prediction, answers = aggregate_signals(
                ...
                reference_keep_matrix,
                reference_signals, 
                np.full(num_data, True), # Note : it can be changed to np.full(membership.shape, True) with top_k=25000 and results should be very close. No knowledge of OUT members is required in order for the attack to work.
                configs,
                kernel=pdistances
            )
            ...
            np.savez(f"{log_dir}/{configs['audit']['report_log']}/scores_gamma_{gamma}",
            ...
            plt.grid(True)
            plt.legend()
            plt.savefig(f"{log_dir}/{configs['audit']['report_log']}/score_distribution.png", bbox_inches='tight')
            plt.clf()
            ...
            np.savez(f"{log_dir}/{configs['audit']['report_log']}/attack_predictions",
                fpr_list=fpr_list,
                tpr_list=tpr_list,
            ...
            plot_roc(
                ...
                tpr_list,
                roc_auc,
                f"{log_dir}/{configs['audit']['report_log']}/{configs['audit']['signal']}_model_attack.png",
            )
