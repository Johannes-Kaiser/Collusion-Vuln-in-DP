...
import torch
from opacus_new.accountants import create_accountant, create_individual_accountant
from opacus_new.accountants.utils import (get_noise_multiplier, get_weights,
                                      assign_pp_values)
from opacus_new.data_loader import (DPDataLoader, switch_generator,
                                IndividualDPDataLoader, IndexedDataset)
from opacus_new.distributed import DifferentiallyPrivateDistributedDataParallel \
    as DPDDP
from opacus_new.grad_sample import (AbstractGradSampleModule, GradSampleModule,
                                get_gsm_class, wrap_model)
from opacus_new.optimizers import DPOptimizer, get_optimizer_class
from opacus_new.scheduler import _NoiseScheduler
from opacus_new.utils.module_utils import trainable_parameters
...
class PrivacyEngine:
    ...
    def make_private(
            ...
            *,
            ...
            weights_debug: Optional[ndarray] = None
        ...
        if self.individualize is not None:
            if pp_weights is not None:  # if adapt_weights_to_budgets
                ...
                if len(self.weights) != self.n_groups:
                    self.weights = list(np.sort(weights_debug))
